...
from utinydb import TinyDB, where, Query
from utinydb.middlewares import Middleware, CachingMiddleware
from utinydb.storages import MemoryStorage, JSONStorage
from utinydb.table import Document
...
def test_insert_multiple(db: TinyDB):
    ...
    db.insert_multiple(
        [{"int": 1, "char": "a"}, {"int": 1, "char": "b"}, {"int": 1, "char": "c"}]
    )
...
def test_insert_multiple_with_doc_ids(db: TinyDB):
    ...
    assert db.insert_multiple(
        [Document({"int": 1, "char": "a"}, 12), Document({"int": 1, "char": "b"}, 77)]
    ) == [12, 77]
...
def test_access_storage():
    ...
    assert isinstance(
        TinyDB(storage=CachingMiddleware(MemoryStorage)).storage, CachingMiddleware
    )
